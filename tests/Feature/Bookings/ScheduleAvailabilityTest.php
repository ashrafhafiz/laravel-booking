...
it('accounts for different daily schedule times', function () {
    ...
    Carbon::setTestNow(Carbon::parse('Monday January 2010'));
    $employee = Employee::factory()
        ->has(Schedule::factory()->state([
            'starts_at' => now()->startOfDay(),
            'ends_at' => now()->addYear()->endOfDay(),
...
it('only shows availablity from the current time with an hour in advance', function () {

    Carbon::setTestNow(Carbon::parse('Monday January 2010 10:40:00'));
    $employee = Employee::factory()
        ->has(Schedule::factory()->state([
            'starts_at' => now()->startOfDay(),
            'ends_at' => now()->addYear()->endOfDay(),
